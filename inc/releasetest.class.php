...
class PluginReleasesReleaseTest extends CommonDBTM {
   ...
   static function getTypeName($nb = 0) {
      return _n('Deployment test', 'Deployment tests', $nb, 'releases');
   }


   static function countForItem(CommonDBTM $item) {
      $dbu = new DbUtils();
      return $dbu->countElementsInTable('glpi_plugin_releases_releasetests',
   ...
   static function canCreate() {
      return Session::haveRight('plugin_releases', CREATE);
      return true;
   }
   ...
   static function canView() {
      ...
      return Session::haveRight('plugin_releases', READ);
      return true;
   }
   ...
   //      $change = new Change();
   //
   //      if ($change->getFromDB($this->fields['plugin_releases_releases_id'])) {
   //         return (Session::haveRight('plugin_releases', UPDATE)
    ...
    **/
   function showInObjectSumnary(CommonDBTM $item, $rand, $showprivate = false) {
      global $DB, $CFG_GLPI;
   ...
   function showForm($ID, $options = []) {
      ...
      if (isset($options['parent']) && !empty($options['parent'])) {
         $item = $options['parent'];
      } else if (isset($options['plugin_releases_releases_id'])) {
         $item = new PluginReleasesRelease();
         $item->getFromDB($options['plugin_releases_releases_id']);
      }
    ...
    *
    * @param $item   CommonDBTM
    **/
   function showSummary(CommonDBTM $item, $id) {
      global $DB, $CFG_GLPI;
      ...
      $query  = "SELECT `id`, `date`
                FROM `glpi_plugin_releases_releasetests`
                WHERE `plugin_releases_releases_id` = '$id'
                      $RESTRICT
                ORDER BY `date` DESC";
